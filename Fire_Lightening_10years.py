    ...
    for each_year in all_years:
        ...
        for each_strike in strike_cursor:
            ...
            if overlap_value == 0 : #Checking for strikes that do not overlap anything
                ...
                #First check to see if it happens in the same year
                if single_fire_date.year == strike_date.year:
                    if days_difference <= DAY_RANGE:
                        struck_new_scar += 1
                    elif strike_date < single_fire_date:
                    ...
                    struck_old_scar += 1
                else: #if not within the same year or 10 years before, it must be nothing
                    struck_outside += 1
                ...
                if num_overlaps == 0:
                    ...
                    for each_fire in fire_cursor:
                        single_fire_date = each_fire.getValue(fire_date_field)
                    days_difference = abs(single_fire_date - strike_date).days # calculates number of days between the two
                    if single_fire_date.year == strike_date.year:
                        if days_difference <= DAY_RANGE:
                            struck_new_scar += 1
                    ...
                    for each_fire in fire_cursor:
                        ...
                        #print "fire", fire_date.year,"vs", "strike", strike_date.year,
                        if fire_date.year == strike_date.year:
                            if days_difference <= DAY_RANGE:
                                new_in_sample = True
                                #print "new"
                            elif strike_date > fire_date:
                                same_year_after = True
                                #print "after"
                            elif strike_date < fire_date:
                                same_year_before = True
                                #print "before"
                        elif fire_date < strike_date: ##It is already the correct year range, so no worries here about checking for years
                            old_in_sample = True
                            #print "old"
                        else:
                            pass
